...
data "archive_file" "lambda_exporter" {
  ...
  source_dir  = var.source_dir #"${path.module}/panda-layer/" #"${data.null_data_source.wait_for_lambda_exporter.outputs["source_dir"]}" #"${path.module}/panda-layer/" # local.source_dir  # 
  type        = "zip"
